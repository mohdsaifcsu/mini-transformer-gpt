# gpt2_model_mini.py
# GPT-2 MINI (8-layer) model — upgraded from tiny version
...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads):
        ...
        # Linear projections for Q, K, V (packed)
        self.qkv = nn.Linear(embed_dim, embed_dim * 3)
        self.out_proj = nn.Linear(embed_dim, embed_dim)

        # causal mask (up to max length = 2048)
        self.register_buffer("mask", torch.tril(torch.ones(2048, 2048)))

    def forward(self, x):
        B, T, C = x.shape

        # project Q,K,V at once then split
...
class GPT2Mini(nn.Module):
    def __init__(
        ...
        vocab_size,
        block_size=128,
        embed_dim=256,    # ↑ bigger than tiny
        ...
        # 8 transformer blocks (instead of 4)
